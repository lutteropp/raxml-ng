#ifndef RAXML_ABSTRACT_TREE_HPP_
#define RAXML_ABSTRACT_TREE_HPP_
...
class AbstractTree : public BasicTree
{
public:
  AbstractTree() : BasicTree(0), _pll_utree(nullptr) {}
  AbstractTree(unsigned int tip_count, const pll_unode_t& root) :
    BasicTree(tip_count),
    _pll_utree(pll_utree_wraptree(pll_utree_graph_clone(&root), tip_count)) {}
  AbstractTree(const pll_utree_t& pll_utree) :
    BasicTree(pll_utree.tip_count), _pll_utree(pll_utree_clone(&pll_utree)) {}
  AbstractTree(std::unique_ptr<pll_utree_t>&  pll_utree) :
    BasicTree(pll_utree ? pll_utree->tip_count : 0), _pll_utree(pll_utree.release()) {}
  AbstractTree(std::unique_ptr<pll_utree_t>&&  pll_utree) :
    BasicTree(pll_utree ? pll_utree->tip_count : 0), _pll_utree(pll_utree.release()) {}

  AbstractTree (const AbstractTree& other);
  AbstractTree& operator=(const AbstractTree& other);
  AbstractTree (AbstractTree&& other);
  AbstractTree& operator=(AbstractTree&& other);

  virtual ~AbstractTree();

  static AbstractTree buildRandom(size_t num_tips, const char * const* tip_labels, unsigned int random_seed);
  static AbstractTree buildRandom(const NameList& taxon_names, unsigned int random_seed);
  static AbstractTree buildRandomConstrained(const NameList& taxon_names, unsigned int random_seed,
                                     const AbstractTree& constrained_tree);
  static AbstractTree buildParsimony(const PartitionedMSA& parted_msa, unsigned int random_seed,
                             unsigned int attributes, unsigned int * score = nullptr);
  static AbstractTree loadFromFile(const std::string& file_name);
...
typedef std::vector<AbstractTree> TreeList;
...
public:
  ...
  void push_back(double score, const AbstractTree& tree);
...
#endif /* RAXML_ABSTRACT_TREE_HPP_ */
