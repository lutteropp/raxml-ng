#include "NetworkBootstopCheck.hpp"

using namespace std;

NetworkBootstopCheck::NetworkBootstopCheck(size_t max_bs_networks)
{
  _max_bs_networks = max_bs_networks;
  _num_bs_networks = 0;
  _pll_splits_hash = nullptr;
}

NetworkBootstopCheck::~NetworkBootstopCheck ()
{
  if (_pll_splits_hash)
    pllmod_utree_split_hashtable_destroy(_pll_splits_hash);
}

void NetworkBootstopCheck::add_bootstrap_network(const Network& network)
{
  if (_num_bs_networks == _max_bs_networks)
  {
    throw runtime_error("NetworkBootstopCheck::add_bootstrap_network: "
        "Maximum number of bootstrap networks reached: " + to_string(_max_bs_networks));
  }
  ...
  for (size_t i = 0; i < network.num_splits(); ++i)
  {
    ...
    if (e->bip_number ==_split_occurence.size())
      _split_occurence.emplace_back(bitVector(_max_bs_networks));
//        _split_occurence.emplace_back(bitVector());

    _split_occurence[e->bip_number][_num_bs_networks] = true;
//    _split_occurence[e->bip_number].set(_num_bs_networks);
  }
...
splitEntryVector NetworkBootstopCheck::all_splits()
{
...
bool NetworkBootstopCheck::converged(unsigned long random_seed)
{
...
NetworkBootstopCheckMRE::NetworkBootstopCheckMRE(size_t max_bs_networks, double cutoff,
                                   size_t num_permutations) : NetworkBootstopCheck(max_bs_networks),
                                       _wrf_cutoff(cutoff), _num_permutations(num_permutations),
                                       _avg_wrf(0.), _avg_pct(0.), _num_better(0)
{
}

NetworkBootstopCheckMRE::~NetworkBootstopCheckMRE ()
{
}

bool NetworkBootstopCheckMRE::check_convergence(RandomGenerator& gen)
{
  bool converged = false;
  const auto num_splits = _pll_splits_hash->entry_count;
  uintVector perm(_num_bs_networks);
  ...
  for (size_t i = 0; i < _num_bs_networks; ++i)
    perm[i] = i;
  ...
  for (size_t p = 0; p < _num_permutations; ++p)
  {
    ...
    for (const auto e: splits_all)
    {
      ...
      for(size_t j = 0; j < _num_bs_networks; j++)
      {
        if (occ[j])
        {
          if(perm[j] % 2 == 0)
    ...
    auto half_split_count = 0.5 * _num_bs_networks * (cons1_splits.size() + cons2_splits.size());
...
void NetworkBootstopCheckMRE::mre(splitEntryVector& splits_all, const uintVector& support,
                           splitEntryVector& splits_cons)
{
  auto mr_support_cuttoff   = _num_bs_networks / 4;
...
double NetworkBootstopCheckMRE::consensus_wrf_distance(const splitEntryVector& splits1,
