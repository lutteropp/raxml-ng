...
RFDistCalculator::RFDistCalculator (const NetworkList& networks, bool lowmem) :
    _num_trees(0), _num_tips(0), _avg_rf(0.0), _avg_rrf(0.0), _num_uniq_trees(0)
{
  if (networks.size() > 1)
  {
    _num_trees = networks.size();
    _num_tips = networks.at(0).num_tips();
    if (lowmem)
      calc_rfdist_lowmem(networks);
    else
      calc_rfdist(networks);
  }
  else
    throw runtime_error("Need at least 2 networks to compute RF distances! "
                        "Given: " + to_string(networks.size()));
}
...
void RFDistCalculator::add_network_splits(size_t tree_idx, const Network& network,
                                       bitv_hashtable_t * splits_hash)
{
  assert(splits_hash);
  assert(network.num_tips() == _num_tips);
  assert(tree_idx < _num_trees);
  ...
  {
    bitv_hash_entry_t * e = pllmod_unetwork_split_hashtable_insert_single(splits_hash,
                                                                       splits[i],
                                                                       1.0);
    if (!e)
      libpll_check_error("Cannot add a split into hashtable: ");
  ...
  pllmod_unetwork_split_destroy(splits);
}
...
void RFDistCalculator::calc_rfdist(const NetworkList& networks)
{
  bitv_hashtable_t * splits_hash = pllmod_unetwork_split_hashtable_create(_num_tips, 0);
  ...
  for (size_t i = 0; i < _num_trees; ++i)
  {
    add_network_splits(i, networks.at(i), splits_hash);
  }
  ...
  pllmod_unetwork_split_hashtable_destroy(splits_hash);
...
void RFDistCalculator::calc_rfdist_lowmem(const NetworkList& networks)
{
  assert(networks.size() > 0);

  /* extract splits from all networks */
  std::vector<pll_split_t *> splits(_num_trees);
  for (size_t i = 0; i < _num_trees; ++i)
  {
  ...
  for (size_t i = 0; i < _num_trees-1; ++i)
  {
    ...
    for (size_t j = i+1; j < _num_trees; ++j)
    {
      auto rf = pllmod_unetwork_split_rf_distance(splits[i], splits[j], _num_tips);
      _avg_rf += rf;
  ...
  for (auto s: splits)
    pllmod_unetwork_split_destroy(s);
