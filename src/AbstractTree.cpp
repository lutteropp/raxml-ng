...
AbstractTree::AbstractTree (const AbstractTree& other) : BasicTree(other._num_tips),
    _pll_utree(other.pll_utree_copy()), _partition_brlens(other._partition_brlens)
{
}

AbstractTree::AbstractTree (AbstractTree&& other) : BasicTree(other._num_tips), _pll_utree(other._pll_utree.release())
{
  other._num_tips = 0;
...
AbstractTree& AbstractTree::operator=(const AbstractTree& other)
{
  if (this != &other)
  {
...
AbstractTree& AbstractTree::operator=(AbstractTree&& other)
{
  if (this != &other)
  {
    _num_tips = 0;
...
void AbstractTree::pll_utree(unsigned int tip_count, const pll_unode_t& root)
{
  _num_tips = tip_count;
...
AbstractTree AbstractTree::buildRandom(size_t num_tips, const char * const* tip_labels,
...
AbstractTree AbstractTree::buildRandom(const NameList& taxon_names, unsigned int random_seed)
{
  ...
  return AbstractTree::buildRandom(taxon_names.size(), (const char * const*) tip_labels.data(),
                           random_seed);
}

AbstractTree AbstractTree::buildRandomConstrained(const NameList& taxon_names, unsigned int random_seed,
                                  const AbstractTree& constrained_tree)
{
...
AbstractTree AbstractTree::buildParsimony(const PartitionedMSA& parted_msa, unsigned int random_seed,
...
AbstractTree AbstractTree::loadFromFile(const std::string& file_name)
{
  AbstractTree tree;
...
PllNodeVector const& AbstractTree::tip_nodes() const
{
...
std::vector<const char*> AbstractTree::tip_labels_cstr() const
{
...
NameList AbstractTree::tip_labels_list() const
{
  NameList result;
...
IdNameVector AbstractTree::tip_labels() const
{
  IdNameVector result;
  for (auto const& node: tip_nodes())
...
NameIdMap AbstractTree::tip_ids() const
{
  NameIdMap result;
  for (auto const& node: tip_nodes())
...
void AbstractTree::insert_tips_random(const NameList& tip_names, unsigned int random_seed)
{
  _pll_utree_tips.clear();
...
void AbstractTree::reset_tip_ids(const NameIdMap& label_id_map)
{
...
void AbstractTree::fix_missing_brlens(double new_brlen)
{
  pllmod_utree_set_length_recursive(_pll_utree.get(), new_brlen, 1);
}

void AbstractTree::reset_brlens(double new_brlen)
{
  pllmod_utree_set_length_recursive(_pll_utree.get(), new_brlen, 0);
}

PllNodeVector AbstractTree::subnodes() const
{
...
TreeTopology AbstractTree::topology() const
{
  TreeTopology topol;
...
void AbstractTree::topology(const TreeTopology& topol)
{
...
const doubleVector& AbstractTree::partition_brlens(size_t partition_idx) const
{
  return _partition_brlens.at(partition_idx);
}

void AbstractTree::partition_brlens(size_t partition_idx, const doubleVector& brlens)
{
  _partition_brlens.at(partition_idx) = brlens;
}

void AbstractTree::partition_brlens(size_t partition_idx, doubleVector&& brlens)
{
  _partition_brlens.at(partition_idx) = brlens;
}

void AbstractTree::add_partition_brlens(doubleVector&& brlens)
{
  _partition_brlens.push_back(brlens);
}

void AbstractTree::apply_partition_brlens(size_t partition_idx)
{
...
void AbstractTree::apply_avg_brlens(const doubleVector& partition_contributions)
{
...
void AbstractTree::reroot(const NameList& outgroup_taxa, bool add_root_node)
{
...
void TreeCollection::push_back(double score, const AbstractTree& tree)
{
