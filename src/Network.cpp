...
Network::Network (const Network& other) : BasicNetwork(other._num_tips),
    _pll_rnetwork(other.pll_rnetwork_copy()), _partition_brlens(other._partition_brlens)
{
}

Network::Network (Network&& other) : BasicNetwork(other._num_tips), _pll_rnetwork(other._pll_rnetwork.release())
{
  other._num_tips = 0;
  swap(_pll_rnetwork_tips, other._pll_rnetwork_tips);
  seap(_pll_rnetwork_reticulations, other._pll_rnetwork_reticulations);
...
Network& Network::operator=(Network&& other)
{
  if (this != &other)
  {
    _num_tips = 0;
    _pll_rnetwork_tips.clear();
    _pll_rnetwork_reticulations_clear();
...
{
  return _pll_rnetwork ? _pll_rnetwork->inner_count : BasicNetwork::num_inner();
}
...
{
  _num_tips = tip_count;
  _pll_rnetwork.reset(pll_rnetwork_wrapnetwork_multi(pll_rnetwork_graph_clone(&root), _num_tips, 0));
  _pll_rnetwork_tips.clear();
  _pll_rnetwork_reticulations.clear();
}

Network Network::buildRandom(size_t num_tips, const char * const* tip_labels,
...
Network Network::buildRandom(const NameList& taxon_names, unsigned int random_seed)
{
  ...
  return Network::buildRandom(taxon_names.size(), (const char * const*) tip_labels.data(),
                           random_seed);
}

Network Network::buildRandomConstrained(const NameList& taxon_names, unsigned int random_seed,
                                  const Network& constrained_network)
{
  PllRNetworkUniquePtr pll_rnetwork(pllmod_rnetwork_resolve_multi(&constrained_network.pll_rnetwork(),
                                                         random_seed, nullptr));
...
Network Network::buildParsimony(const PartitionedMSA& parted_msa, unsigned int random_seed,
                           unsigned int attributes, unsigned int * score)
{
  ...
  PllRNetworkUniquePtr pll_rnetwork(pllmod_rnetwork_create_parsimony_multipart(taxon_count,
...
{
  if (_pll_rnetwork_reticulations.empty() && _num_reticualations > 0)
  {
...
void Network::insert_tips_random(const NameList& tip_names, unsigned int random_seed)
{
  ...
  int retval = pllmod_rnetwork_extend_random(_pll_rnetwork.get(),
                                          tip_labels.size(),
...
PllNetworkNodeVector Network::subnodes() const
{
  ...
  if (_num_tips > 0)
  {
    ...
    for (size_t i = 0; i < _pll_rnetwork->tip_count + _pll_rnetwork->inner_tree_count + _pll_rnetwork->reticulation_count; ++i)
    {
      auto start = _pll_rnetwork->nodes[i];
      auto node = start;
      do
      {
...
NetworkTopology Network::topology() const
{
  ...
  for (auto n: subnodes())
  {
    ...
    {
      topol.edges.at(n->pmatrix_index) = NetworkBranch(n->node_index, n->back->node_index, n->length);
      branches++;
    }
  }
...
void Network::topology(const NetworkTopology& topol)
{
  ...
  for (const auto& branch: topol)
  {
    ...
    pllmod_rnetwork_connect_nodes(left_node, right_node, branch.length);
...
void Network::reroot(const NameList& outgroup_taxa, bool add_root_node)
{
  ...
  int res = pllmod_rnetwork_outgroup_root(_pll_rnetwork.get(), tip_ids.data(), tip_ids.size(),
                                       add_root_node);
...
{
  return std::max_element(_networks.cbegin(), _networks.cend(),
